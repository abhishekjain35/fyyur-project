...
def search_venues():
  ...
  for venue in res:
    finalData.append({
      "id": venue.id,
      "name": venue.name,
      "num_upcoming_shows": len(Show.query.filter(venue_id=venue.id).filter(Show.start_time > datetime.now()).all())
    })
...
def search_artists():
  ...
  for artist in res:
    finalData.append({
      "id": artist.id,
      "name": artist.name,
      "num_upcoming_shows": len(Show.query.filter(artist_id=artist.id).filter(Show.start_time > datetime.now()).all())
    })
